...
def get_ongoing_classes_list(today):
   ...
   # with open('ongoing_classes1_2022_with_level.sql', 'r') as file:
    ...
    with open(f'ongoing_classes_list/ongoing_obem_classes_{today}.csv', 'w', newline='') as file:
...
def get_required_level(today):
    ...
    ongoing = pd.read_csv(f'ongoing_classes_list/ongoing_obem_classes_{today}.csv')
    ...
    with open(f'required_level/required_level_{today}.csv', 'w', newline='') as file:
        csv_writer = csv.writer(file)
        csv_writer.writerow([i[0] for i in cursor.description])
        csv_writer.writerows(cursor.fetchall())
